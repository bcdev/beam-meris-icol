 ...
 */
@OperatorMetadata(alias = "Meris.CoastDistance",
        version = "1.0",
        internal = true,
        authors = "Olaf Danne",
        copyright = "(c) 2009 by Brockmann Consult",
        description = "Coast distance computation.")
public class MerisCoastDistanceOp extends MerisBasisOp {
    ...
    public void initialize() throws OperatorException {
    	targetProduct = createCompatibleProduct(l1bProduct, "coast_distance_"+l1bProduct.getName(), "COASTD");
                ...
                for (int x = targetRectangle.x; x < targetRectangle.x + targetRectangle.width; x++) {
                    if (isWater.getSampleBoolean(x, y)) {
                        ...
                        if (lineEndPix.x == -1 || lineEndPix.y == -1) {
                            ...
                            if (landPix != null) {
                            	coastDistance.setSample(x, y, (int) NavigationUtils.distanceInMeters(geocoding, startPix, landPix));
                            } else {
                            	coastDistance.setSample(x, y, NO_DATA_VALUE);
                            }
                        }
...
//                         coastDistance.setSample(x, y, NO_DATA_VALUE);               what's this???
    ...
    public static class Spi extends OperatorSpi {
        public Spi() {
            super(MerisCoastDistanceOp.class);
        }
    }
}
