 ...
 */
@OperatorMetadata(alias = "Tm.EtmAeMerge",
                  internal = true,
                  version = "1.1",
...
public class EtmAeMergeOp extends TmBasisOp {
    ...
    private Band[] addBandGroup(String prefix, double noDataValue) {
        return OperatorUtils.addBandGroup(sourceProduct, TmConstants.LANDSAT7_NUM_SPECTRAL_BANDS, new int[]{5, 6}, targetProduct, prefix, noDataValue, false);
    }
    ...
    public void computeTile(Band band, Tile targetTile, ProgressMonitor pm) throws OperatorException {
        ...
        try {
            ...
            if (bandName.startsWith(EtmAeMergeOp.AE_TOTAL) &&
                ...
                Tile aeAerosol = getSourceTile(aeAerosolProduct.getBand("rho_aeAer_" + bandNumber), rectangle,
                                               BorderExtender.createInstance(BorderExtender.BORDER_COPY));
                Tile aeMaskRayleigh = getSourceTile(aeMaskRayleighProduct.getBand(AdjacencyEffectMaskOp.AE_MASK_RAYLEIGH), rectangle,
                                                    BorderExtender.createInstance(BorderExtender.BORDER_COPY));
                Tile aeMaskAerosol = getSourceTile(aeMaskAerosolProduct.getBand(AdjacencyEffectMaskOp.AE_MASK_AEROSOL), rectangle,
                                                   BorderExtender.createInstance(BorderExtender.BORDER_COPY));
